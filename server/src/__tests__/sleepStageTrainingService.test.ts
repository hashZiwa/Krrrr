...
describe("sleepStageTrainingService", () => {
  it("loads CSV files from a rawdata directory and trains a model", async () => {
    ...
    const validationDataDir = await mkdtemp(join(tmpdir(), "sleeper-validation-"));
    await writeFile(
    ...
    await writeFile(
      join(validationDataDir, "validation.csv"),
      [
        "timestamp,sleep_stage,sleep_stage_code,respiratory_rate_bpm",
        "2026-07-20 01:20:17,Wake,40001,18",
        "2026-07-20 01:21:17,REM,40004,16",
        "2026-07-20 01:22:17,Light,40002,14",
        "2026-07-20 01:23:17,Deep,40003,11",
      ].join("\n"),
      "utf8",
    );
